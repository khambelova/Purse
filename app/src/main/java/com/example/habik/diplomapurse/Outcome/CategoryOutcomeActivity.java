...
import com.example.habik.diplomapurse.Outcome.DataAdapterOutcome;
...
public class CategoryOutcomeActivity extends AppCompatActivity implements View.OnClickListener{
    List<Category> categoriesOut = new ArrayList<>();
    ImageButton imgBtnAddCategory;
    EditText addCategoryET;
    String catName;
    String oldCatName;
    RecyclerView recyclerView;
    DataAdapterOutcome adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = CategoryOutcomeActivity.this;
        dbHelper = new DBHelper(this);
        database = dbHelper.getWritableDatabase();


        adapter = new DataAdapterOutcome(this, categoriesOut, new DataAdapterOutcome.OnItemClickListener() {
            @Override
            public void onItemClick(final Category item) {
                ...
                builder.setNegativeButton("Удалить", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        database.delete(PurseContract.OutcomeCategoriesEntry.TABLE_OUTCOME_CATEGORY,PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME + " = ?",new String[]{item.getNameCategory().toString()});
                        categoriesOut.remove(item);
        ...
        Cursor cursor = database.query(PurseContract.OutcomeCategoriesEntry.TABLE_OUTCOME_CATEGORY, null, null, null, null, null, null);
        if (cursor.moveToFirst()) {
            int nameIndex = cursor.getColumnIndex(PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME);
            Category category = new Category(cursor.getString(nameIndex));
            categoriesOut.add(category);

            while (cursor.moveToNext()) {
                nameIndex = cursor.getColumnIndex(PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME);
        ...
        switch (v.getId()) {
            case R.id.btn_save: {
                if (isEdited == true) {
                    ...
                    contentValues.put(PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME, catName);
                    database.update(PurseContract.OutcomeCategoriesEntry.TABLE_OUTCOME_CATEGORY, contentValues,PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME
                            + " = ?" ,new String[]{oldCatName});
                    addCategoryET.setText("");
                    categoriesOut.set(position - 1,new Category(catName));
                    adapter.notifyDataSetChanged();
                    isEdited = false;
                }
                else{
                    contentValues.put(PurseContract.OutcomeCategoriesEntry.OUTCOME_CATEGORY_NAME, catName);
                    database.insert(PurseContract.OutcomeCategoriesEntry.TABLE_OUTCOME_CATEGORY, null, contentValues);
                    categoriesOut.add(new Category(catName));
                    addCategoryET.setText("");
                    adapter.notifyDataSetChanged();}
                break;
            }
