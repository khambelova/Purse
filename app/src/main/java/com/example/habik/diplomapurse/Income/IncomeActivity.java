...
public class IncomeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    TextView incomeDate,categoryText;
    ImageButton dateImBtng, categoryImgBtn, saveImgBtn;
    EditText incomeSum, incomeComment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        incomeDate = findViewById(R.id.date_income);
        ...
        incomeSum = findViewById(R.id.sum);
        incomeComment = findViewById(R.id.comment);
        Intent intent = getIntent();
        String date = intent.getStringExtra("date_income");
        incomeDate.setText(date);
        textInputLayout = findViewById(R.id.text_input_layout);
        incomeComment = (EditText) textInputLayout.findViewById(R.id.comment);
        ...
        if(isEdited)
        {
            ...
            incomeDate.setText(getIntent().getStringExtra("date"));
            incomeSum.setText(String.valueOf(getIntent().getDoubleExtra("sum", 0.0)));
            incomeComment.setText(getIntent().getStringExtra("comment"));
    ...
    AdapterView.OnItemSelectedListener onItemSelectedListener() {
        return new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(currency!= null && !incomeSum.getText().toString().equals("")) {
                    sumInRub = currencyDict.get(currency) * Double.parseDouble(incomeSum.getText().toString());
                }
            }
    ...
    public void onClick(View view) {
        ...
        if(sumInRub == 0 && currency!= null && !incomeSum.getText().toString().equals("")) {
            sumInRub = currencyDict.get(currency) * Double.parseDouble(incomeSum.getText().toString());
        }
        ...
        switch (view.getId()) {
            case R.id.date_imgBtn: {
                Intent intent_data = new Intent(IncomeActivity.this,CalendarActivity.class);
                startActivityForResult(intent_data,REQUEST_CODE_DATA);
                break;
            }
            case R.id.category_imgBtn: {
                Intent intent_category = new Intent(IncomeActivity.this,CategoryIncomeActivity.class);
                startActivityForResult(intent_category,REQUEST_CODE_CATEGORY);
                break;
            }
            case R.id.btn_save: {
                    contentValues.put(PurseContract.IncomeEntry.INCOME_DATE, incomeDate.getText().toString());
                    contentValues.put(PurseContract.IncomeEntry.INCOME_CATEGORY, categoryText.getText().toString());
                    contentValues.put(PurseContract.IncomeEntry.INCOME_SUM, Double.parseDouble(incomeSum.getText().toString()));
                    contentValues.put(PurseContract.IncomeEntry.INCOME_CURRENCY, currency);
                    contentValues.put(PurseContract.IncomeEntry.INCOME_COMMENT, incomeComment.getText().toString());
                    contentValues.put(PurseContract.IncomeEntry.INCOME_SUM_IN_RUB, sumInRub);
                    if(!isEdited) {
                        database.insert(PurseContract.IncomeEntry.TABLE_INCOME, null, contentValues);
                    }
                    else {
                        database.update(PurseContract.IncomeEntry.TABLE_INCOME, contentValues,
                                PurseContract.IncomeEntry._ID + "= ?", new String[]{String.valueOf(index)});
                    }
                    sumInRub = 0;
        ...
        if(resultCode ==RESULT_OK) {
            switch (requestCode){
                case REQUEST_CODE_DATA:{
                    ...
                    incomeDate.setText(calendarDate);
                    break;
                }
